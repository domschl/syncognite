...
class WordEmbedding : public Layer {
  private:
    ...
    void setup(const json &jx) {
        ...
        cppl_set(&params, "W",
                 new MatrixN(xavierInit(MatrixN(V, D), inittype, initfactor)));
        wrVect = MatrixN(V, V);
        ...
        /*
        params["W"]->setRandom();
        floatN xavier = 1.0/std::sqrt((floatN)(inputShapeFlat+D)); // (setRandom
        is [-1,1]-> fakt 0.5, xavier is 2/(ni+no)) *params["W"] *= xavier;
        */
        layerInit = true;
    }
  ...
  public:
    ...
    */
    virtual MatrixN forward(const MatrixN &x, t_cppl *pcache, t_cppl *pstates,
        ...
        int N = shape(x)[0];
        MatrixN W(*params["W"]);
        // MatrixN wrVect(*params["V"]);
        MatrixN xv(N * T, V);
        for (int n = 0; n < N; n++) {
            for (int t = 0; t < T; t++) {
                for (int vi = 0; vi < V; vi++) {
                    if (x(n, t) >= V) {
                        cerr << "WordEmbedding: Internal error: x(n,t)="
                             << x(n, t) << ">=V:" << V << endl;
                        exit(-1);
                    }
                    xv(n * T + t, vi) = wrVect((int)x(n, t), vi);
                }
            }
        }
        if (pcache != nullptr)
            cppl_set(pcache, "xv", new MatrixN(xv));
        MatrixN y0 = xv * W;
        MatrixN y(N, D * T);
        for (int n = 0; n < N; n++) {
            for (int d = 0; d < D; d++) {
    ...
    virtual MatrixN backward(const MatrixN &dchain, t_cppl *pcache,
        ...
        // dW = np.dot(p1.T, p2)
        // cerr << "N:" << N << ", T:" << T << ", V:" << V << ", D:" << D <<
        // endl;  cerr << "dW:" << shape(dW) << ", xv:" << shape(xv) << ",
        // dchain:" << shape(dchain) << endl;
        MatrixN dct(D, N * T);
        for (int n = 0; n < N; n++) {
            for (int t = 0; t < T; t++) {
                for (int d = 0; d < D; d++) {
