...
// XXX: This implementation is faulty and needs to be completely rewritten.
...
class RAN : public Layer {
  private:
    ...
    void setup(const json &jx) {
        ...
        N = j.value("N", 1);
        XavierMode inittype =
            xavierInitType(j.value("init", (string) "standard"));
        initfactor = j.value("initfactor", (floatN)1.0);
        maxClip = j.value("clip", (float)0.0);
        nocupdate =
            j.value("nocupdate", (bool)false);  // true for auto-diff tests
        D = inputShape[0];
        T = inputShape[1];
        outputShape = {H, T};
        ...
        cppl_set(
            &params, "Wxh",
            new MatrixN(xavierInit(MatrixN(D, 3 * H), inittype, initfactor)));
        cppl_set(
            &params, "Whh",
            new MatrixN(xavierInit(MatrixN(H, 2 * H), inittype, initfactor)));
        cppl_set(
            &params, "bh",
  ...
  public:
    ...
    MatrixN Sigmoid(const MatrixN &m) {
        ...
        // Alternative via tanh is stable and doesn't need case distinctions for
        // large + - inf values of x. alt see:
    ...
    t_cppl forward_step(const MatrixN &x, t_cppl *pcache, t_cppl *pstates,
                        int id = 0) {
        t_cppl cp;
        MatrixN cprev = *(*pstates)[cname];
        MatrixN xhx1 = cprev * *params["Whh"];
        MatrixN xhx2 =
            (x * *params["Wxh"]).rowwise() + RowVectorN(*params["bh"]);
    ...
    virtual MatrixN backward_step(t_cppl cp, t_cppl *pcache, t_cppl *pstates,
                                  t_cppl *pgrads, int id = 0) {
        ...
        // MatrixN xhx2 = (x * *params["Wxh"]).rowwise() +
        // RowVectorN(*params["bh"]); cerr << shape(*params["Wxh"]) <<
        // shape(dxhx.transpose()) << shape(x) << endl;
        ...
        // MatrixN xhx1 = cprev * *params["Whh"];
        cerr << "dc:" << shape(*params["Whh"]) << shape(dxhx1) << shape(dcnext)
             << endl;
        MatrixN dc = (*params["Whh"] * dxhx2.transpose()).transpose();
        cerr << "dWhh:" << shape(cprev) << shape(dxhx1) << shape(*params["Whh"])
             << endl;
        ...
        */
        ...
        if (maxClip != 0.0) {
            ...
            *(*pgrads)["Wxh"] =
                (*(*pgrads)["Wxh"]).array().min(maxClip).max(-1 * maxClip);
            *(*pgrads)["Whh"] =
                (*(*pgrads)["Whh"]).array().min(maxClip).max(-1 * maxClip);
            *(*pgrads)["bh"] =
                (*(*pgrads)["bh"]).array().min(maxClip).max(-1 * maxClip);
            *(*pgrads)[cname0] =
                (*(*pgrads)[cname0]).array().min(maxClip).max(-1 * maxClip);
        }
    ...
    virtual MatrixN backward(const MatrixN &dchain, t_cppl *pcache,
        ...
        for (int t = T - 1; t >= 0; t--) {
            ...
            // cp[cname]=new MatrixN(dci); // XXX: dchain should also take care
            // of dc! -> zero-init for now.
            dxi = backward_step(cp, &cache, &states, &grads, id);
            cppl_delete(&cp);
            cerr << "dx,dxi,N,T,D:" << shape(dx) << shape(dxi) << N << "," << T
                 << "," << D << ";" << t << endl;
