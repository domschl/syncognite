  ...
  public:
    ...
    virtual floatN loss(t_cppl *pcache, t_cppl *pstates) override {
        ...
        if (y.rows() != probs.rows() || y.cols() != 1) {
            cerr << layerName << ": "
                 << "Loss, dimension mismatch in Softmax(x), Probs: ";
            cerr << shape(probs) << " y:" << shape(y) << " y.cols=" << y.cols()
                 << "(should be 1)" << endl;
            return 1000.0;
        }
        // if (pcache!=nullptr) cppl_set(pcache, "y", new MatrixN(y));
        ...
        for (unsigned int i = 0; i < probs.rows(); i++) {
            if (y(i, 0) >= probs.cols()) {
                cerr << "internal error: y(" << i << ",0) >= " << probs.cols()
                     << endl;
                return -10000.0;
            }
            floatN pr_i = probs(i, (int)y(i, 0));
            if (pr_i == 0.0)
