...
bool checkTemporalAffineForward(floatN eps=CP_DEFAULT_NUM_EPS) {
    ...
    x << 1.11915759, -1.02276588, -0.40875152, -0.25741233,
          0.03933653,  0.46370403,  0.38811546,  0.84503295,
         -2.53860058, -0.96297824, -1.22945235,  1.31878163,

         -1.2467802 ,  0.85933785, -0.27288343,  1.5230758,
          0.91333914, -1.30205627,  1.99936087, -0.46547715,
         -1.41231122,  0.04041277, -0.51678583, -0.43403757;
    MatrixN W(5,6); // (D,M)
    W << 0.35857381,  0.70593925, -0.52236759,  1.12165522,  2.10087442, 0.33569053,
         1.42807963, -1.25879138,  0.38812581,  0.52462091,  0.01797542, -0.01465269,
        -0.44958498, -1.61586571,  0.33579688,  0.75798716, -1.30222583, 0.16518008,
         0.29647818, -0.80113616, -0.5913838 , -0.56425738,  0.55482746, -0.1246292,
        -0.83981768, -0.26583433,  0.44778482,  1.65494828,  0.41845642, 1.04776263;
    MatrixN b(1,5);
    b << -0.17642297,  0.19639896, -1.16824051,  0.25422063, -0.19417997;
    MatrixN y(2,3*5); // (N, T*M)
    y << 1.43845182, -0.49450765, -2.42745454,  0.39087218,  0.42004851,
         -2.06707566, -0.14179877, -0.01729424,  0.93898115, -1.00991961,
          1.09570378,  2.03159689, -1.61333228,  2.14666807,  1.20983306,

         -2.4295828 ,  0.82802536,  0.58088417,  2.03098666, -0.85481721,
         -0.1417757 , -0.94611035, -2.0254132 , -1.83361151, -0.33992646,
          1.07148488,  1.49946998, -1.62572005, -0.06598739,  0.59768283;
...
bool checkTemporalAffineBackward(float eps=CP_DEFAULT_NUM_EPS) {
    ...
    *(pe.params["b"])=b;
    ...
    MatrixN dx(2,3*4);
    dx << 0.69861904, -1.55932749, -0.01121794, -0.73443243,
          0.41569649,  0.51173684, -0.57028666,  1.16105616,
         -0.87231076,  1.60495371,  0.44522648,  1.5709303,

          0.63932023, -0.35985699, -0.26702418,  0.52566605,
         -0.16018798, -0.62825849,  1.13096181, -2.00377255,
          0.78239434,  1.8555152 ,  1.17447251,  1.27293577;
    MatrixN dW(5,6);
    dW << -0.01839845,  10.46481322,   0.84784387,  -6.36378141,
         -2.12966998,  -6.2460433,
         -1.3359454 ,  -0.23207504,  -1.90352171,   7.22867192,
         12.97291737,  -3.13700014,
          1.75722602,  11.76195693,   1.67408517,   7.26335475,
         -0.5706298 ,  10.88660798,
         -0.38231025,   2.04966874,  -6.19431692,  -9.76589008,
          1.18942076,   3.18927005,
         -3.38351315,  -0.594996  ,  -3.35491894,  -7.35504654,
         -10.63804151,   5.94751517;
    MatrixN db(1,5);
    db << -1.56594737, -1.59197068, -0.36824766, -0.19226169, -0.03567894;
    MatrixN dchain(2,15);
    dchain << -0.52548557, -1.22102464, -1.61584428, -0.17234407,  0.91849498,
         0.42952029, -0.17406755,  0.4876361 ,  0.89514373, -0.72626425,
        -1.34625958,  1.36076687,  0.47386774,  0.29178916,  0.76017646,

        -0.11756133, -0.99992855, -0.06805038,  0.39461432,  0.67850665,
         0.03065535, -0.4377662 , -0.0111982 , -1.64959446,  0.61240684,
        -0.03681652, -0.11995061,  0.36534135,  0.04812964, -2.27899963;
...
int doTests() {
    ...
    *(we.params["W"])=weW;
    ...
    // N=10; T=5; D=6; M=7
    TemporalAffine pct(CpParams("{inputShape=[30];hidden=20;T=5;D=6;M=7}")); // 30=T*D
    MatrixN xtt(10,30);
    xtt.setRandom();
