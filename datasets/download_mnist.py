...
def loadOriginalFiles(datadict, destpath):
    ...
    for du in datadict:
        ...
        if os.path.isfile(destfile) is False:
            ...
            return False
        if os.stat(destfile).st_size != du["size"]:
            print("Error:", destfile, " was downloaded, but has unexpected size!")
            return False
    return True
...
def encodeOriginalsToH5(datadict, zippath, h5path):
    ...
    for du in datadict:
        ...
        for ds in du["datasets"]:
            ...
            dset = hf.create_dataset(ds, shape, dtype=dtype, compression='gzip')
            dset[...] = di
    hf.close()
...
loadOriginalFiles(mnist_urls, destpath)
encodeOriginalsToH5(mnist_urls, destpath, localpath)
